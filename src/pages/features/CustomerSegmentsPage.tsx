...
import { Search, Menu, X, Users, Plus } from 'lucide-react';
        ...
        <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-1">
            ...
            <div className="space-y-1 pl-2">
              ...
              <button onClick={() => navigate('/feature/customer-segments')} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-lg bg-[#243118] text-[#89b825] border-l-4 border-[#89b825] hover:bg-[#243118]/90 transition text-left">
        ...
        <header className="flex items-center justify-between h-20 px-6 bg-white border-b border-stone-200">
          <div className="flex items-center flex-1 max-w-lg gap-4">
            ...
            <div className="relative w-full">
              ...
              <input type="text" placeholder="Search rules..." className="w-full h-11 pl-11 pr-4 text-sm bg-[#f3e9dd] border-0 rounded-full text-stone-800 outline-none" />
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8">
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex items-center gap-2 text-xs font-semibold text-stone-500">
              ...
              <span>/</span>
              <span className="text-[#89b825]">Customer Segments</span>
            </div>

            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-extrabold text-slate-900">Automated Customer Segments</h1>
                <p className="text-sm text-slate-500 mt-1">Configure automated rules for grouping boutique clientele.</p>
              </div>
              <button className="flex items-center gap-2 px-5 py-2.5 bg-[#101e12] hover:bg-[#1c301f] text-[#89b825] text-sm font-bold rounded-xl shadow transition">
                <Plus size={16} />
                <span>Create Segment Rule</span>
              </button>
            </div>

            <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm space-y-4 text-left">
              <div className="p-4 bg-stone-50 rounded-xl border border-stone-150 flex justify-between items-center">
                <div>
                  <h4 className="font-bold text-sm text-slate-900">LTV Threshold &gt; ₹1,00,000</h4>
                  <p className="text-xs text-slate-500">Automatically assigns Gold Member tier badge upon threshold reach.</p>
                </div>
                <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-bold">Active</span>
              </div>
              <div className="p-4 bg-stone-50 rounded-xl border border-stone-150 flex justify-between items-center">
                <div>
                  <h4 className="font-bold text-sm text-slate-900">Try-On Conversion &gt; 30%</h4>
                  <p className="text-xs text-slate-500">Flags shopper profile for priority bridal catalog preview.</p>
                </div>
                <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-bold">Active</span>
              </div>
            </div>
